...
const MainPage = () =>{
    ...
    useMemo(()=>{
        if(searchQuery !== ''){
        axios.get('https://www.googleapis.com/books/v1/volumes?q='+searchQuery+'+subject:'+chooseCateg+'+orderBy:'+chooseSort+'&key=AIzaSyCbchQ4B-QN4WzoKXCZEGCOSNtvN3HDY78&maxResults=36')
    ...
    return(
        <div style={{width:'100%'}}>
            ...
            <p style={{textAlign:'center', marginBottom: '20px'}}>{(typeof(books) !== 'undefined')?books.length:0} results</p>
            {openBook === false
                ? <>{(searchQuery !== '' && typeof(books) !== 'undefined')
                    ?<BookList books={pagbooks()} forOpenBook={forOpenBook}/>
                    :<h1 style={{fontSize:'76px', textAlign: 'center', marginTop: '30vh'}}>¯\_(ツ)_/¯</h1>}
                    </>
